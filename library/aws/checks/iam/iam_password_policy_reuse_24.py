    ...
    def execute(self, connection: boto3.Session) -> CheckReport:
        ...
        try:
            ...
            if password_reuse_prevention is None:
                ...
                report.resource_ids_status["Password reuse prevention not configured"] = False
            elif 1 <= password_reuse_prevention <= 24:
                # Valid range is 1-24 as per AWS documentation
                report.resource_ids_status[f"Password policy prevents reuse of the last {password_reuse_prevention} passwords"] = True
            else:
                report.status = ResourceStatus.FAILED
                report.resource_ids_status[f"Password reuse prevention value {password_reuse_prevention} is outside the valid range (1-24)"] = False
