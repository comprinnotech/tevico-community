...
def test_check_report(report: CheckReport):
    ...
    # If resource_ids_status is empty, allow PASSED, UNKNOWN, or NOT_APPLICABLE
    if not report.resource_ids_status:
        assert report.status in [CheckStatus.PASSED, CheckStatus.UNKNOWN, CheckStatus.NOT_APPLICABLE]
        return
    ...
    elif all(resource.status == CheckStatus.NOT_APPLICABLE for resource in report.resource_ids_status):
